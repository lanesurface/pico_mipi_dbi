...
static _Bool _mgl_evt_tk_loop_running;
...
_mgl_evt_tick_loop (void)
{
	...
	while (_mgl_evt_tk_loop_running) {
		async_context_poll (_async_ctx);

		now=get_time_ms ();
		_Bool b_locked=mutex_try_enter (&_evt_tk_mtx, NULL);
		if (b_locked) {
			delta_tm=(last_tm-now);
			nd=_evt_tk_cbs;
			while (nd) {
				nd->evt_tk_cb (
					nd->gfx_ctx, // << mgl_gfx_ctx * gfx_ctx_
					delta_tm
				);
				nd=(nd->next);
			}
			mutex_exit (&_evt_tk_mtx);
		} else {
			/**
...
mgl_suspend_evt_tick (void)
{
	...
	b_lock=mutex_enter_timeout_ms (&_evt_tk_mtx, MIPI_MAX_TM);
	if (b_lock) {
		_mgl_evt_tk_loop_running=false;
		mutex_exit (&_evt_tk_mtx);
		...
		_mipi_dbg (
			MIPI_DGB_TAG,
			"failed to lock evt_tk core, aborting"
		);
	}
 ...
 * updates to the screen once per tick. Because it is called by the graphics
 * subsystem, no lock need be held on the context or frame buffer, so that
 * such updates may take place without blocking.
 *
	...
	mgl_evt_tick_cb evt_tick_cb )
{
	...
	if (nd) {
		...
		_Bool b_locked=mutex_enter_timeout_ms (
			&_evt_tk_mtx,
			MIPI_MAX_TM
		);
		...
		if (b_locked) {
			...
			_mipi_dbg (
				MIPI_DBG_TAG,
				"stalled acquiring lock for `_evt_tk_cbs`"
			);
		}
	} else {
		_mipi_dbg (
...
static void 
_mgl_init_fmbf_tx (struct mgl_gfx_ctx * self);

static void 
_mgl_render_gfx_objs (struct mgl_gfx_ctx * self);

void 
mgl_mark_fmbf_dirty (struct mgl_gfx_ctx * gfx_ctx)
{
...
void 
mgl_exec_task_in_bkgd (mgl_bkgd_task_cb bkgd_tsk)
{
...
{
	/**
	 * TODO: Set up and initialize the async context and workers.
	 */
 ...
 */
void
mgl_reset_evt_tick_subsys (void)
{
	...
	/**
	 * From this point on, there is no reason to acquire the `_evt_tk_mtx` 
	 * because all code running on core 1 has stopped. It is thus safe
	 * to alter this state without locking.
	 */
	struct _mgl_evt_tk_ll_node * nd, * tmp;
	nd=_evt_tk_cbs;
