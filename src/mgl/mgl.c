...
static volatile /* _OSAL_ATOMIC_BOOL */ atomic_bool
_evt_tk_running;
#else
/**
 * The mutex itself should enforce memory order consistency, but defined as a
 * volatile-qualified type anyway.
 */
...
_is_evt_tick_running (void)
{
	#ifdef __STDC_NO_ATOMICS__
		_Bool b_lock=__osal_mutex_acquire_block_ms (&_loop_stat_mtx, MIPI_MAX_TM);
		if (!b_lock) {
	...
	mgl_evt_tick_cb evt_tick_cb )
{
	...
	nd=malloc (sizeof(*nd)); // _osal_thrd_safe_malloc (...);
	if (nd) {
		...
		if (b_lock) {
			...
			_mipi_dbg (
				MIPI_DBG_TAG,
				"stalled acquiring lock for `_tk_cbs_mtx`"
			);
		}
	} else {
		_mipi_dbg (
...
mgl_reset_evt_tick_subsys (void)
{
	...
	/**
	 * From this point on, there is no reason to acquire the `_evt_tk_mtx`because
	 * all code running on core 1 has stopped. It is thus safe to alter this
